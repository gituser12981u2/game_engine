...
MeshHandle MeshStore::createMesh(VkUploadContext::Recorder staticRecorder,
  ...
  if (!staticRecorder) {
    std::cerr << "[MeshStore] createMesh requires a valid static recorder\n";
    return {};
  }
  ...
  if (!m_uploader.uploadToDeviceLocalBuffer(staticRecorder, vertices, vbSize,
  ...
  if (indices != nullptr && indexCount > 0) {
    ...
    if (!m_uploader.uploadToDeviceLocalBuffer(staticRecorder, indices, ibSize,
...
MeshHandle MeshStore::createMesh(VkUploadContext::Recorder staticRec,
                                 const engine::MeshData &mesh) {
  return createMesh(staticRec, mesh.vertices.data(),
